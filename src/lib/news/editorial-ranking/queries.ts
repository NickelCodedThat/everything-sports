...
function toView(row: Row): EditorialItemView {
  ...
  const se = (r.section_eligibility ?? {}) as {
    lead?: boolean;
    wire?: boolean;
    now?: boolean;
    desk?: boolean;
  ...
  return {
    ...
    activeOverrides:
      (r.active_overrides as EditorialItemView["activeOverrides"]) ?? [],
...
export async function listItems(
  ...
  if (query.eligibility?.length)
    request = request.in("eligibility", query.eligibility);
...
export async function getItem(
  client: WarehouseClient,
  idOrClusterId: string,
): Promise<EditorialItemView | null> {
  ...
  const byCluster = await client
    .from("editorial_items")
    .select("*")
    .eq("cluster_id", idOrClusterId)
    .maybeSingle();
  if (byCluster.error)
    throw new Error(`getItem failed: ${byCluster.error.message}`);
...
export function toSlateItem(view: EditorialItemView): SlateItem {
  ...
  return {
    ...
    entityKeys: view.entities.filter(
      (e) => e.startsWith("team:") || e.startsWith("name:"),
    ),
...
): Promise<{ slate: Slate; items: Map<string, EditorialItemView> }> {
...
export async function getSupportingSources(
  client: WarehouseClient,
  view: Pick<EditorialItemView, "clusterId">,
): Promise<SupportingSource[]> {
  const { data: cluster, error: clusterError } = await client
    .from("story_clusters")
    .select("representative_candidate_id")
    .eq("id", view.clusterId)
    .maybeSingle();
  if (clusterError)
    throw new Error(`getSupportingSources failed: ${clusterError.message}`);
  ...
  const data = await readPages(
    (from, to) =>
      client
        ...
        .select(
          "candidate_id, news_candidates!inner(headline, headline_kind, source_url, published_at, discovered_at, news_sources(domain, display_name, quality_bucket, is_enabled))",
        )
        .eq("cluster_id", view.clusterId)
        .order("candidate_id")
        .range(from, to),
    "getSupportingSources failed",
  );
  ...
  for (const row of data ?? []) {
    ...
    if (
      !current ||
      source.isRepresentative ||
      (!current.isRepresentative &&
        (source.publishedAt?.getTime() ?? Infinity) <
          (current.publishedAt?.getTime() ?? Infinity))
    )
  ...
  return toStoryPreview(
    {
      ...
      eligibility: view.eligibility,
      eligibilityReasons: view.eligibilityReasons,
    },
    sources,
  );
}
...
export async function setStatus(
  ...
  const { data, error } = await client.rpc("editorial_set_status", {
    p_item: itemId,
    p_status: status,
  ...
  if (error)
    throw new Error(error.message.replace(/^editorial_set_status: /, ""));
...
export async function setOverride(
  ...
  if (
    input.kind === "boost" &&
    !(input.amount && input.amount > 0 && input.amount <= OVERRIDE.boostMax)
  )
    throw new Error(`boost amount must be between 1 and ${OVERRIDE.boostMax}`);
  if (
    input.kind === "suppress" &&
    input.amount !== undefined &&
    !(input.amount > 0 && input.amount <= 1000)
  )
    throw new Error("suppress amount must be between 1 and 1000");
  const { data, error } = await client.rpc("editorial_set_override", {
    p_item: itemId,
    p_kind: input.kind,
    p_amount:
      input.kind === "suppress"
        ? (input.amount ?? OVERRIDE.suppressDefault)
        : (input.amount ?? undefined),
  ...
  if (error)
    throw new Error(error.message.replace(/^editorial_set_override: /, ""));
  return Number(data);
}
...
export async function removeOverride(
  ...
  const { data, error } = await client.rpc("editorial_remove_override", {
    p_item: itemId,
    p_kind: (kind ?? null) as never,
    p_reason: reason ?? undefined,
    p_actor: actor,
  });
