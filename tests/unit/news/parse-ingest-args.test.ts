...
describe("engine report formatting", () => {
  it("prints the health report without secrets and with the fields an operator needs", async () => {
    ...
    const tick = formatTickReport({ trigger: "scheduled", startedAt: "a", finishedAt: "b", reaped: { count: 1, runIds: ["x"] }, ok: true, clustering: null, ranking: null, results: [{ providerId: "gdelt", outcome: "skipped-not-scheduled", detail: "manual-only" }] });
